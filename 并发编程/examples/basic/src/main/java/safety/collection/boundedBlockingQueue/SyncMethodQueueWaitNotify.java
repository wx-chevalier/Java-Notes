package safety.collection.boundedBlockingQueue;
 ...
 *    1. releases safety.lock on q
 *    2. sleeps ( gives up processor )
 *    3. awakens ( resumes running )
 *    4. reacquires safety.lock and returns
 *    
 ...
 *       2. Producer C enqueues and signals notEmpty, waking A. Before A can acquire the safety.lock, however, another producer D puts a second item in the queue,
 *          and because the queue is not empty, it does not signal notEmpty
 *       3. Then A acquires the safety.lock, removes the first item, but B, victim of a lost wakeup, waits forever even though there is an item in the buffer to be consumed.
 * 
 * how to fix the bug:
