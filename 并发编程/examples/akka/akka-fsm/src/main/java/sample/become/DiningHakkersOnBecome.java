  ...
  public static class Chopstick extends AbstractActor {
    ...
    Receive takenBy(ActorRef hakker) {
      return receiveBuilder()
        .match(Messages.Take.class, t -> t.hakker.tell(new Busy(self()), self()))
        .match(Messages.Put.class, p -> p.hakker == hakker,
   ...
   */
   public static class Hakker extends AbstractActor {
    ...
    Receive eating = receiveBuilder()
      .matchEquals(Think, m -> {
        left.tell(new Messages.Put(self()), self());
        right.tell(new Messages.Put(self()), self());
    ...
    Receive waitingFor(ActorRef chopstickToWaitFor, ActorRef otherChopstick) {
      return receiveBuilder()
        ...
        match(Busy.class, b -> {
          otherChopstick.tell(new Messages.Put(self()), self());
    ...
    Receive deniedAChopstick = receiveBuilder()
      .match(Taken.class, t -> {
        t.chopstick.tell(new Messages.Put(self()), self());
        startThinking(Duration.create(10, MILLISECONDS));
      })
      .match(Busy.class, b ->
        startThinking(Duration.create(10, MILLISECONDS)))
    ...
    Receive thinking = receiveBuilder()
      .matchEquals(Messages.Eat, m -> {
        getContext().become(hungry);
        left.tell(new Messages.Take(self()), self());
        right.tell(new Messages.Take(self()), self());
      }).build();

    private void startThinking(FiniteDuration duration) {
      getContext().become(thinking);
      getContext().system().scheduler()
        .scheduleOnce(duration, self(), Messages.Eat, getContext().system().dispatcher(), self());
    }
  }
